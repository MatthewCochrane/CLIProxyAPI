...
func (m *Manager) updateInternal(ctx context.Context, base, auth *Auth, mode updateAuthMode) (*Auth, error) {
	...
	auth.EnsureIndex()
	// Minted Meta keys and rotated Codex credentials must reach the configured
	// store before requests can use them. A Codex refresh can rotate the refresh
	// token, so installing it first could leave an obsolete token on disk.
	...
	persistBeforeInstall := (mode == updateModePrepare || mode == updateModeRefresh) && strings.EqualFold(strings.TrimSpace(auth.Provider), "meta") ||
		mode == updateModeRefresh && strings.EqualFold(strings.TrimSpace(auth.Provider), "codex")
	var persistWarning error
	if persistBeforeInstall {
		if errPersist := m.persist(ctx, auth); errPersist != nil {
			committedStore, committed := m.store.(interface{ SaveErrorCommitted(error) bool })
			if !committed || !committedStore.SaveErrorCommitted(errPersist) {
				m.mu.Unlock()
				action := "refreshed"
				if mode == updateModePrepare {
					action = "prepared"
				}
				return nil, fmt.Errorf("persist %s auth: %w", action, errPersist)
			}
			persistWarning = errPersist
		}
	}
	authClone := auth.Clone()
	m.auths[auth.ID] = authClone
	...
	if persistWarning != nil {
		return auth.Clone(), fmt.Errorf("persist refreshed auth: %w", persistWarning)
	}
	return auth.Clone(), nil
}
